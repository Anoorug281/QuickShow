                ...
                {nowPlayingMovies.map((movie)=>(
                    ...
                    duration-300`} onClick={()=>setSelectedMovie(movie.id)}>
                        ...
                        {selectedMovie === movie.id && (
                            ...
                            items-center justify-center bg-primary h-6 w-6 rounded'>
        ...
        <div className='mt-6'>
            ...
            <ul className='space-y-3'>
                {Object.entries(dateTimeSelection).map(([date,times])=>(
                    <li key={date}>
                        <div className='font-medium'>{new Date(date).toDateString()}</div>
                        <div className='flex flex-wrap gap-2 mt-1 text-sm'>
                            {times.map((time)=>(
                                <div key={time} className='border border-primary
                                px-2 py-1 flex items-center rounded'>
                                    <span>{time}</span>
                                    <Trash2Icon onClick={()=> handleRemoveTime(date,time)}
